...
每家会拿到四个只读工具——`list_files` / `read_file` / `grep_repo` 作用于你唤起会诊时
所在的那个仓，外加 `web_search` 联网查证。它们存在的理由：**查不了的模型会编。**
两场真实会诊里核过 6 条"实测"断言，**4 条是编的**，而且语气和真事实一模一样。

仓库工具治的是关于你代码的编造，`web_search` 治的是关于世界的编造——库的版本号、
某个 API 存不存在、引用的出处到底怎么说——**而之前编得最凶的正是后面这一类**。

`web_search` 复用你已经存好的 `ollama` key，不用再申请任何东西；没有那个 key 时它会
返回一句明确的"不可用"，而不是崩掉。

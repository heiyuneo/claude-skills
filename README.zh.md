...
2. **`NO_PROXY='ollama.com'`**：skill 里的命令都带着它。没挂代理的机器上完全无害，
   ...
   **它为什么管用值得知道，否则手动排障会被带偏**：Python 只在**一个 `*_proxy` 变量都没有**
   时才去读 macOS 系统代理，而 `NO_PROXY` 小写成 `no_proxy`、以 `_proxy` 结尾，算命中。
   所以设了它就**整个跳过系统代理**，跟你在里面写的是哪个域名无关。
   「带 `NO_PROXY` 就通、不带就 `Connection error`」**既不是限流，也不是那个域名的事**。
...
4. **`~/.claude/panel-runs/` 是全机共享的**，不分项目也不分会话。`check-run.sh` 不带参数
   验的是**最近一次**——两个会话同时在跑时，那是**别人那一跑**，却按你的报给你。
   **显式传归档目录。** 思考档、key、阵容同理：都存在 `llm` 的配置里，改一次全机生效。
...
`web_search` 会按你配好的后端顺序挨个查，并标明是哪个索引答的——见[搜索用哪家](#搜索用哪家)。
`web_fetch` 永远留在 Ollama 那边，因为 Brave 只给摘要不给正文。

**查你的仓库** —— `list_files` / `read_file` / `grep_repo`，范围限定在发起会诊的那个仓。
没有它们的话，问题包就是五家能看到的全部——**等于一个人决定了五个模型眼里的整个世界**，
而这个人恰好也是最后做裁决的那个。

这三个工具**上过一次线又撤了，现在是重建不是恢复**。当初用来挡秘密的黑名单被一份
`.worktrees/` 副本绕了过去，所以现在**根本没有黑名单**——可见范围由 `git ls-files`
**正向定义**，没进版本控制的东西对这些工具根本不存在。当初的搜索用 `pathlib.glob`，
花 85 秒对着**明明存在**的内容答"没找到"；**报假阴性的工具比没有工具更糟**，因为模型
接着就会用"我查过了"的口气断言不存在。现在底座是 `git grep`，每个空结果都必须说
"搜过了，是空的"，`setup/panel_tools.py` 里带着专测这条回归的自检和完整的来龙去脉。
...
| `minimax` | **单独关掉工具**。它在三次大包会诊里都崩了，但诊断**不是**"工具调用坏"——单次搜索在各个思考档位都是几秒钟就过。真正复现出来的是**不收敛**：让它查四件事，它会拿几乎相同的词反复重搜。关掉工具反而两次交出全场最长的答案，而且它成了唯一**纯靠问题包推理**的对照组。**现在它那一份包会额外声明"你没有工具"**：公共包里写着"请优先用检索核实"，而它是唯一不带工具启动的，于是整回合都在宣布要检索、调不出任何东西，停在 788 字节、exit 0 |
| `glm` | **五家里唯一跑在 `high`，这是刻意的。** 在 `max` 档它会拖过 720 秒预算被本地杀掉——但**被丢弃的那笔请求仍占着智谱的并发位**，于是**下一次跑**一开局就是 `429`、零字节、`llm logs` 里连记录都没有（请求根本没进模型）。7 次跑这样丢了 5 次。**入口是超时，不是 429**，所以重试是错的杆，跑进预算才是对的。缺席的评审员论证为零——而那正是顶档本该买到的东西 |
...
这一版改了 `setup/panel-doctor.py`，光更新 plugin 不够：
...
python3 "$CFG/panel-doctor.py" --set-effort max      # glm 会被刻意压到 high，它自己会说明
```

**你的 key、端点、阵容都不动。** 改了这些：

- **glm 降到 `high`。** 在 `max` 档它拖过扇出预算被杀，而那笔死掉的请求还占着供应商的并发位，
  于是**下一次跑**一开局就 `429`——7 次里这样丢了 5 次。`--set-effort max` 现在会自动压住它
  并打印原因；真要顶档就加 `--only glm`。详见「各家的已知脾气」里它那一行。
- **立场矩阵每个格子都要带原文引用，验收器会把每一条 grep 回那个模型的答案文件。**
  你只会读汇总，所以**汇总层自己的误引率**是这套设计从没测过的那个数字——评审员层测到过 6 条里 4 条编造。
  第一次真跑：51 条引用全部命中；在副本里改坏两条，检查会失败，所以这个"通过"是有含义的。
- **快失败会重试一次。** 限流拒绝、或 `exit 0` 但正文短到不可用——这两种几秒钟就回来，
  不重试就白丢一个座位。**超时被杀的故意不重试**：那说明它工具循环烧穿了预算，再跑一次还是一样。
- **minimax 会被告知它没有工具**，因为它是唯一不带工具启动的那个座位。

如果你自己改过 `check-run.sh`：实际用法上它现在要接归档目录做参数，见「四个坑」第 4 条。
